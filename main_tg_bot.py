...
from source_keys import OPENAI_API_KEY, ASSISTANT_ID_152FZ, TELEGRAM_152FZ_TOKEN  # Импортируем необходимые ключи и токены из локального модуля src
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "👋 Привет! Я бот-юрист - помощник по 152-ФЗ и перс.данным.\n"
        "Отправь описание своего сервиса и получи его анализ от OpenAI."
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        run = client.beta.threads.runs.create(
            thread_id=thread_id,
            assistant_id=ASSISTANT_ID_152FZ
        )
...
# --- Запуск бота ---
def main():
    app = ApplicationBuilder().token(TELEGRAM_152FZ_TOKEN).build()  # Инициализируем приложение Telegram бота с токеном
